document.addEventListener('DOMContentLoaded',inicio());let map;let edificiosJSON;function inicio(){crearSlider();if(sessionStorage.getItem('date')===null)
    ...
    sessionStorage.setItem('i','0');leerJSONEdificios().then(()=>{crearSeccionPortfolio()});getUserPosition();setNavPorfolioHidden(!1);crearSeccionTeam();if(!document.getElementById('submitSuccessMessage')){crearContactoYFooter()}
    let date=new Date(sessionStorage.getItem('date'));document.getElementById('Copyright').innerHTML="Copyright &copy; Mallorca Route "+date.getFullYear();let aLogo=document.getElementById('logoMashorca');let aInicio=document.getElementById('inicio');aLogo.onclick=goHome;aInicio.onclick=goHome;scriptSlider()}
